...
local fruitRespawnEffectEvent = ReplicatedStorage:WaitForChild(FRUIT_RESPAWN_EFFECT_EVENT_NAME)
...
local function playRespawnEffect()
    ...
    if playSound(RESPAWN_SOUND_ID, RESPAWN_VOLUME, "respawn") then
        addRecentLog("[RespawnEffect] sound playing")
    end

    if playSound(RESPAWN_JINGLE_SOUND_ID, RESPAWN_JINGLE_VOLUME, "respawn_jingle") then
        addRecentLog("[RespawnEffect] jingle playing")
    end
